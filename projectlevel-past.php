        ...
        <div class="section-card">
            ...
            <h2 class="card-header"><?php echo __('project_badge_history'); ?></h2>
                <div id="projectList" class="project-list p-3">
                    <?php
                    // 檢查是否存在當前專案
                    $hasCurrentProject = isset($_SESSION['gbd_project_id']) && !empty($_SESSION['gbd_project_id']);
                    
                    if (!$hasCurrentProject) {
                        // 如果沒有當前專案，顯示提示訊息
                        echo "<div class='alert alert-info'>請先創建或選擇綠建築專案，才能查看相關的歷史篩選組別</div>";
                    } else {
                        // 如果有當前專案，繼續顯示該專案的歷史記錄
                        if (isset($_SESSION['user_id'])) {
                            $userID = $_SESSION['user_id'];
                            $gbdProjectId = $_SESSION['gbd_project_id'];
                            ...
                            $countSql = "
                                ...
                                FROM pj_filters 
                                WHERE UserID = :userID AND building_id = :buildingId
                            ";
                            ...
                            // 查詢該使用者的所有篩選組別，必須加入 building_id 過濾條件
                            $sql = "
                                SELECT 
                                    ProjectName,
                            ...
                            try {
                                ...
                                $stmt->bindValue(':buildingId', $gbdProjectId);
                                ...
                                if (empty($projects)) {
                                    echo "<div class='alert alert-info'>當前專案下尚無儲存的篩選組別</div>";
                                } 
                                else {
                                    // 顯示專案列表
