    ...
    <div class="content-wrapper">
        ...
        <div class="section-card">
            <div class="input-section" id="inputSection">
                ...
                <div>
                    <button class="button" onclick="validateAndInitialize()">確認尺寸</button>
                </div>
            </div>
        </div>
        ...
        <div id="drawingSection">
            ...
            <div class="project-name-display">
                <h3>目前專案: <span id="currentProjectName">空白專案</span></h3>
            </div>
                <h2>工具列</h2>
                <div class="controls">
                <button class="button" onclick="setDrawMode('polygon')">🖊️ 繪製多邊形</button>
                <button class="button" onclick="setDrawMode('height')">🏗️ 修改建物高度</button>
                <button class="button" onclick="clearCanvasWithConfirm()">🧽 清除畫布</button>
                <button class="button" onclick="resetArea()" style="background-color:rgb(212, 157, 38);">🔄 重設專案</button>
                <button class="button" onclick="exportToExcel()">📊 導出Excel</button>
                <button class="button" onclick="saveProject()">💾 儲存專案</button>
                <button class="button" onclick="saveAsProject()">📝 另存專案</button>
                <button class="button" onclick="loadProjectList()">📂 載入專案</button>
                </div>
    ...
    function initializeGrid() {
        ...
        function exportToExcel() {
            ...
            shapes.forEach((shape, index) => {
                data.push({
                    '形狀編號': index + 1,
                    '形狀類型': shape.type === 'rectangle' ? '矩形' : '多邊形',
                    '面積': calculateArea(shape).toFixed(2),
                    '高度': shape.zHeight ? `${shape.zHeight} ${lengthUnit}` : '未設定'
                });
            });
            ...
            for (let i = 0; i < shapes.length; i++) {
                for (let j = i + 1; j < shapes.length; j++) {
                    distanceData.push({
                        '形狀編號': `${i + 1} - ${j + 1}`,
                        '形狀類型': '邊緣距離',
                        '距離': calculateEdgeDistance(shapes[i], shapes[j]).toFixed(2)
                    });
                }
            }

            // 建立並下載Excel檔案
            const ws = XLSX.utils.json_to_sheet([...data, ...distanceData]);
            const wb = XLSX.utils.book_new();
            XLSX.utils.book_append_sheet(wb, ws, "街廓數據");
            XLSX.writeFile(wb, "街廓數據.xlsx");
        }
        // 添加事件監聽器
